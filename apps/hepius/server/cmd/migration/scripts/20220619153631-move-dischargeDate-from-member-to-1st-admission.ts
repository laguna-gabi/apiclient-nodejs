...
export const up = async (dryRun: boolean, db: Db) => {
  ...
  const membersRaw = await db.collection('members').find({ dischargeDate: { $exists: 1 } });
   ...
   * so we'll :
   * 1. if admission exists for a member, we'll append the dischargeDate on the same admission found.
   * 2. if admission does not exist for a member, we'll create an admission with the dischargeDate.
   */
  await Promise.all(
  ...
  await db
    .collection('members')
    .updateMany({ dischargeDate: { $exists: true } }, { $unset: { dischargeDate: 1 } });
};

export const down = async (dryRun: boolean, db: Db) => {
  const app = await NestFactory.createApplicationContext(AppModule);
  const memberModel = app.get<Model<Member>>(getModelToken(Member.name));
  const admissionsRaw = await db.collection('admissions').find({ dischargeDate: { $exists: 1 } });
  ...
  await Promise.all(
    admissions.map(async (admission) => {
      await memberModel.updateOne(
        ...
        { $set: { dischargeDate: admission.dischargeDate } },
      );
    }),
  );
