...
export const up = async (dryRun: boolean, db: Db) => {
  ...
  await recordingModel.updateMany({}, { $set: { consent: false } });
  const appointmentsWithRecordingConsent = await appointmentModel.find({ recordingConsent: true });
  await Promise.all(
    appointmentsWithRecordingConsent.map(async (appointment) => {
...
export const down = async (dryRun: boolean, db: Db) => {
  ...
  await recordingModel.updateMany({ consent: { $exists: true } }, { $unset: { consent: 1 } });
};
