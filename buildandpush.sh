...
# get npm token - this does not work you might need to paste manualy the token in the variable, might be fixed in the future.
readonly npm_secret=$(sed -e 's|//npm.pkg.github.com/:_authToken=\(.*\)|\1|' ~/.npmrc)
# readonly npm_secret=

# build the image
docker build -t ${repo} --build-arg GIT_COMMIT=${git_commit} --build-arg GIT_BRANCH=${git_branch} --build-arg NPM_TOKEN=${npm_secret} .
